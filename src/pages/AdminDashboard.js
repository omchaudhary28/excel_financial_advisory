...
import { FiUsers, FiMessageSquare, FiStar, FiMail, FiPhone } from "react-icons/fi";
...
function AdminDashboard() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><LoadingSpinner text="Loading Admin Dashboard..." /></div>;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 bg-white dark:bg-gray-800 rounded-2xl shadow-xl">
      ...
      {queries.map((q) => (
        <tr key={q.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          ...
          <td className="px-6 py-4 flex items-center gap-4">
            ...
            {q.user_phone && (
              <a href={`tel:${q.user_phone}`} className="text-primary hover:underline inline-flex items-center gap-1"><FiPhone/> Call</a>
            )}
          </td>
        </tr>
      ))}
...
const TableRatings = ({ feedback }) => (
  <table className="w-full text-left table-auto">
    ...
    <tbody className="text-gray-700 dark:text-gray-200">
      {feedback.map((f) => (
        <tr key={f.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          <td className="px-6 py-4 font-medium">{f.name}<br/><span className="text-xs text-gray-500 dark:text-gray-400">{f.email}</span></td>
